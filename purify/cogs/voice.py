...
class Voice(commands.Cog):
    ...
    @app_commands.command(name="voicemastersetup", description="Initialize VoiceMaster")
    async def voicemastersetup(self, interaction: discord.Interaction) -> None:
        if not interaction.user.guild_permissions.administrator:
            ...
            return
        await interaction.response.send_message("✅ VoiceMaster setup initialized.", ephemeral=True)

    @app_commands.command(name="greetvoicesetup", description="Set up the greet voice flow")
